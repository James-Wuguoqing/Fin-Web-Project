...
import {
  ...
  NEWS_ARTICLE_ARCHIVE_LIMIT,
  NEWS_ARTICLE_INDEX_KEY,
  NEWS_ARTICLE_RETENTION_MS,
  PRESS_RELEASE_SYMBOLS,
  TWELVE_DATA_API_KEY
} from "./config";
import {
  buildStampedFallback,
  formatChange,
  formatFredValue,
  formatFxValue,
  formatQuoteValue,
...
import { fetchAlphaVantage, fetchFredSeries, fetchStooqSnapshot, fetchTwelveData } from "./providers";
...
async function getHomepageNewsFeed(limit = 4) {
  const feed = await getCachedNewsFeed();
  return feed.slice(0, limit);
}

async function resolveHomepageItem(label, task, fallback) {
  try {
    return await task();
  } catch (error) {
    console.error(`Failed to load homepage ${label}:`, error);
    return fallback;
  }
}

function buildWaitingMacroMetric(label) {
  return {
    label,
    value: "--",
    delta: 0,
...
async function getHomepageDataRaw() {
  try {
    const [spy, qqq, dia, iwm, usdTwd, eurUsd, usdJpy, news, fedFunds, cpi, unemployment] =
      await Promise.all([
        resolveHomepageItem("SPY quote", () => getQuote("SPY", "SPY"), buildWaitingItem("SPY")),
        resolveHomepageItem("QQQ quote", () => getQuote("QQQ", "QQQ"), buildWaitingItem("QQQ")),
        resolveHomepageItem("DIA quote", () => getQuote("DIA", "DIA"), buildWaitingItem("DIA")),
        resolveHomepageItem("IWM quote", () => getQuote("IWM", "IWM"), buildWaitingItem("IWM")),
        resolveHomepageItem("USD/TWD FX", () => getFxRate("USD", "TWD", "USD/TWD"), buildWaitingItem("USD/TWD")),
        resolveHomepageItem("EUR/USD FX", () => getFxRate("EUR", "USD", "EUR/USD"), buildWaitingItem("EUR/USD")),
        resolveHomepageItem("USD/JPY FX", () => getFxRate("USD", "JPY", "USD/JPY"), buildWaitingItem("USD/JPY")),
        resolveHomepageItem("news feed", () => getHomepageNewsFeed(4), buildFallbackData().news),
        resolveHomepageItem(
          "FEDFUNDS metric",
          () => getFredMetric("FEDFUNDS", "联邦基金利率"),
          buildWaitingMacroMetric("联邦基金利率")
        ),
        resolveHomepageItem(
          "CPIAUCSL metric",
          () => getFredMetric("CPIAUCSL", "美国 CPI"),
          buildWaitingMacroMetric("美国 CPI")
        ),
        resolveHomepageItem(
          "UNRATE metric",
          () => getFredMetric("UNRATE", "美国失业率"),
          buildWaitingMacroMetric("美国失业率")
        )
      ]);
